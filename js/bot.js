...
const token = "2053778459:AAEw6FdhOEWu294sJY8h1WoLUYr2P0ozoqI";
...
bot.on('message', (msg, next) => {
  ...
  console.log(msg)
  if (!baza.some(a => a.id == chatId) && msg.chat.id != "-1001662527121") {
      baza.push({
...
bot.on('callback_query', async (query) => {
    ...
    if(query.data == "cells") {
        var text_message = `<b>📂 Информация о покупке ячеек</b>\n<b>🪙 Цена:</b> ${options.price}₽\n<b>🗄 Количество ячеек:</b> ${options.amount}\n\n<b>✅ Доступно ячеек для заполнения:</b> ${numberWithCommas(data[0].cells.hacked)}`
        var buttons = [
        ...
        if(data_pay == "WAITING") {
          ...
          bot.answerCallbackQuery(query.id, {
            ...
            bot.getChatMember(`-1001400522600`, chatId, {}).then(async (data_user) => {
              ...
              send_messages("343783264", messages_text, []);
            })
            send_messages(chatId);
